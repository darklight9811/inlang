...
test("roundtrip: plural and select", async () => {
	const input = {
		en: {
			likes:
				"You have {count, plural, one {# like} other {# likes}}",
			gender:
				"{gender, select, male {He} female {She} other {They}}",
		},
		de: {
			likes:
				"Du hast {count, plural, one {# Like} other {# Likes}}",
			gender:
...
test("roundtrip: strict apostrophe escaping patterns", async () => {
	const input = {
		en: {
			...
			mixed: "''{name}'' is quoted and {count, plural, one {'#' item} other {'#' items}}",
		},
	};
...
async function runImportFiles(messagesByLocale: Record<string, Record<string, string>>) {
